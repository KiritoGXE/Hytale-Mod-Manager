...
install_mod_by_id() {
    ...
    if [[ -z "$download_url" || "$download_url" == "null" ]]; then
        local fpart1=$((file_id / 1000))
        local fpart2=$((file_id % 1000))
        download_url="https://edge.forgecdn.net/files/${fpart1}/${fpart2}/${filename}"
    fi
...
update_mods() {
    ...
    while IFS= read -r filename; do
        ...
        if [[ "$latest_file_id" != "$current_file_id" ]]; then
            ...
            # Fallback CDN: alcuni mod author disabilitano la distribuzione diretta via API
            if [[ -z "$download_url" || "$download_url" == "null" ]]; then
                local upart1=$((latest_file_id / 1000))
                local upart2=$((latest_file_id % 1000))
                download_url="https://edge.forgecdn.net/files/${upart1}/${upart2}/${new_filename}"
            fi
